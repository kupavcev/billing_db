...
select ResourceId as volumeId,ItemDescription as rDescription,username as volumeTag,sum(unBlendedCost) as Cost 
from aws.aws_billing_reports_detailed
	...
	having sum(unBlendedCost)>0
	order by sum(unBlendedCost) desc
end;
